...
export default function VerificationPeriodPicker({
  ...
  const handleCustomDateChange = (date: Date | null) => {
    ...
    if (date) {
      ...
      if (diffDays === 7) {
        ...
        period = "2week"
      } else if (date.getMonth() === today.getMonth() + 1 && date.getFullYear() === today.getFullYear()) {
        period = "1month"
      } else if (date.getMonth() === today.getMonth() + 6 && date.getFullYear() === today.getFullYear()) {
        period = "6months"
  ...
  return (
    <div className="w-full font-urbanist">
      <label htmlFor="period-select" className="font-urbanist font-normal text-base leading-6 align-middle">
      ...
      {selectedPeriod === "custom" && (
        <div className="relative mt-2 w-full">
          ...
          />
          {selectedDate && (
            <p className="text-xs text-gray-400 mt-1">Selected date: {selectedDate.toLocaleDateString()}</p>
          )}
        </div>
      )}
