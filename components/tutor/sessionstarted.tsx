...
const sessionData = [
  {
    id: 1,
    question: 'A customer is experiencing difficulties with our mobile application. What is the first step you should do?',
    options: ['Alert your manager', 'Create a Tech ticket', 'Collect Information'],
    answer:
  ...
  {
    id: 2,
    question: 'A user reports a bug that crashes the app when uploading photos. What should you do first?',
    options: ['Type answer'],
    answer: [
      <span key="1">Not quite! <span className="text-[#F9DB6F]">The correct answer is 24.</span></span>,
        ...
        {sessionData.map((q: SessionQuestion) => (
          <div key={q.id} className="mb-10">
            ...
            <div className="bg-[#222222] rounded-[20px] p-6 mb-6">
              <h2 className=" font-urbanist font-extrabold text-[24px] leading-[100%] mb-4">Question {q.id}:</h2>
              <p className="font-urbanist font-medium text-[24px] leading-[100%] tracking-[0.1px] mb-6">{q.question}</p>
              ...
              <div className="flex-1">
                {Array.isArray(q.answer) ? (
                  q.answer.map((line: ReactNode, i: number) => (
                    typeof line === 'string' ? (
                      <p key={i} className="font-urbanist font-medium text-[20px] leading-[100%] tracking-[0.1px] text-gray-300">{line}</p>
                    ) : (
                      <div key={i} className="font-urbanist font-medium text-[20px] leading-[100%] tracking-[0.1px] text-gray-300">{line}</div>
                    )
                  ))
                ) : (
                  <p className="font-urbanist font-medium text-[20px] leading-[100%] tracking-[0.1px] text-gray-300">{q.answer}</p>
                )}
              </div>
            </div>
          </div>
