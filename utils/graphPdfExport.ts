  ...
  try {
    ...
    for (const section of config.sections) {
      ...
      for (const item of section.content) {
        ...
        yPos +=
          item.type === "summary-stat" ? 6 : item.type === "data-row" ? 5 : 5;
      }
...
export const createTutorAnalyticsPDFConfig = (
  ...
  const validData = chartData.filter(
    (item) => item.hasData && item.actualValue > 0
  );
  ...
  if (validData.length > 0) {
    avgScore =
      validData.reduce((sum, item) => sum + item.actualValue, 0) /
      validData.length;
  ...
  if (
    validData.length > 0 ||
    allDataWithValues.some((item) => item.actualValue >= 0)
  ) {
    // Overview section
    const overviewContent: PDFContent[] = [
      ...
      {
        type: "text",
        text: `Performance analysis covers ${totalMonths} months with ${activeMonths} months showing activity (${(
          (activeMonths / totalMonths) *
          100
        ).toFixed(1)}% activity rate).`,
      },
    ];

    if (maxScore > 0) {
      const bestMonth = chartData.find(
        (month) => month.actualValue === maxScore
      );
    ...
    if (minScore >= 0 && maxScore !== minScore) {
      const lowestMonth = chartData.find(
        (month) => month.actualValue === minScore
      );
      overviewContent.push({
        type: "highlight",
        text: `Lowest Performance: ${lowestMonth?.month} - ${minScore.toFixed(
          1
        )}%`,
        color: "danger",
      });
    ...
    const breakdownContent: PDFContent[] = chartData.map((month, index) => {
      ...
      const scoreText = month.hasData
        ? `${month.actualValue.toFixed(1)}%`
        : "No Data";

      return {
        type: "data-row" as const,
        text: `${month.month}|${scoreText}|${statusText}:${statusColor}${
          trendText ? `|${trendText}:${trendColor}` : ""
        }`,
      };
    });
    ...
    if (recentMonths.length > 0 && earlierMonths.length > 0) {
      const recentAvg =
        recentMonths.reduce((sum, m) => sum + m.actualValue, 0) /
        recentMonths.length;
      const earlierAvg =
        earlierMonths.reduce((sum, m) => sum + m.actualValue, 0) /
      ...
      if (trend > 2) {
        insightsContent.push({
          type: "text",
          text: `• Performance improving by ${trend.toFixed(
            1
          )}% in recent months`,
          indent: true,
        });
        ...
        insightsContent.push({
          type: "text",
          text: `• Performance declining by ${Math.abs(trend).toFixed(
            1
          )}% in recent months`,
          indent: true,
        });
...
export const createCardsAnalyticsPDFConfig = (
  ...
  if (totalCards > 0) {
    const avgCardsPerWeek =
      chartData.length > 0 ? totalCards / chartData.length : 0;
    const maxCardsInWeek =
      chartData.length > 0
    ...
    const overviewContent: PDFContent[] = [
      ...
      {
        type: "text",
        text: `Card creation analysis over 6 weeks with ${activeWeeks} active weeks (${(
          (activeWeeks / 6) *
          100
        ).toFixed(1)}% activity rate).`,
      },
    ...
    const breakdownContent: PDFContent[] = chartData.map((week, index) => {
      ...
      return {
        ...
        text: `${week.name}|${week.value} cards|${statusText}:${statusColor}${
          trendText ? `|${trendText}:${trendColor}` : ""
        }`,
      };
    });
